...
import { getBookings, getConversations, getMessages, getUsers } from "@/lib/db"
...
export default async function AdminNotificationsPage() {
  const [bookings, conversations, messages, users] = await Promise.all([
    getBookings(),
    getConversations(),
    getMessages(),
    getUsers(),
  ...
  const total = pendingBookings.length + unreadConversations.length + newMessages.length + newUsers.length
  ...
  return (
    <div className="space-y-8">
      ...
      <Section title="New Users (24h)" empty="No new users in the last 24 hours." href="/admin/users">
...
function Section({ title, children, empty, href }: { title: string; children: React.ReactNode; empty: string; href: string }) {
